...
void 
dlist_add_frontv(struct dlist *l, void *ptr, size_t size) {
...
void 
dlist_add_backv(struct dlist *l, void *ptr, size_t size) {
	struct dnode *n = dnode_create();
	n->data = ptr;
	n->size = size;
...
void 
dlist_add_back(struct dlist *l, char *ptr) {
	struct dnode *n = dnode_create();
	n->data = malloc(512);
	memcpy(n->data, ptr, strlen(ptr));
	//n->data = ptr;
	if (0 == l->counter) {
		l->front = l->back = n;	
		l->counter = 1;
...
struct dnode
dlist_remove_frontv(struct dlist *l) {
	struct dnode *n = l->front;
	struct dnode ret;
	if (1 == l->counter) {
		l->front = l->back = NULL;
...
struct dnode
dlist_remove_backv(struct dlist *l) {
	struct dnode *n = l->back;
	struct dnode ret;
...
struct dnode
dlist_find_removev(struct dlist *l, void *ptr) {
	struct dnode *n = l->front;
	struct dnode ret;
	ret.data = NULL;
	...
	if (n != NULL) {
	  if (l->front == n) {
	    return dlist_remove_frontv(l);
	  } else if (l->back == n) {
	    return dlist_remove_backv(l);
	  } else {
	    if (1 == l->counter) {
	      l->front = l->back = NULL;
	    } else {
